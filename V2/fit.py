...
if argc >= 9:
    outputsize = int(sys.argv[8])
    print("outputsize:", outputsize)
if argc >= 10:
    epoches = int(sys.argv[9])
...
for i in range(samples): train_y[i] = np.genfromtxt('training_data/vframe-' + str(i) + '.csv', delimiter=',', dtype=float)
...
for i in range(predict_samples):
    np.asarray(p[i]).tofile(outdir_name + '/vframe-' + "{:.2f}".format(sp*float(i)) + '.csv', sep=',')
